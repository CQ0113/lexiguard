...
class _DynamicLegalFormPageState extends State<DynamicLegalFormPage> {
  ...
  final FocusNode _clientSelectorFocusNode = FocusNode();
  String? _selectedClientId;
  List<_ConnectedClientOption> _connectedClientOptions = const [];
  VaultDocumentRepository? _vaultRepositoryInstance;
  VaultDocumentRepository get _vaultRepository => _vaultRepositoryInstance ??=
      widget.vaultRepository ?? VaultDocumentRepository();
  ConnectionRequestRepository? _connectionRequestRepositoryInstance;
  ConnectionRequestRepository get _connectionRequestRepository =>
      _connectionRequestRepositoryInstance ??=
  ...
  void initState() {
    ...
    _clientIdController.addListener(_syncSelectedClientFromText);
  }

  @override
  void dispose() {
    _clientIdController.removeListener(_syncSelectedClientFromText);
    _clientIdController.dispose();
    _clientSelectorFocusNode.dispose();
  ...
  String? _currentLawyerId() {
    final provided = widget.currentLawyerId?.trim();
  ...
  List<_ConnectedClientOption> _clientOptionsFromRequests(
    List<ConnectionRequestModel> requests,
  ) {
    final byClientId = <String, _ConnectedClientOption>{};
    for (final request in requests) {
      if (request.status != ConnectionRequestStatus.approved) continue;
      final clientId = request.clientId.trim();
      if (clientId.isEmpty || byClientId.containsKey(clientId)) continue;
      byClientId[clientId] = _ConnectedClientOption(
        clientId: clientId,
        clientName: _clientDisplayName(request),
        caseTitle: request.caseSnapshot?.title.trim().isNotEmpty == true
            ? request.caseSnapshot!.title.trim()
            : request.caseId,
      );
    }
    return byClientId.values.toList()..sort(
      (left, right) => left.clientName.toLowerCase().compareTo(
        right.clientName.toLowerCase(),
      ),
    );
  }

  String _clientDisplayName(ConnectionRequestModel request) {
    final name = request.clientReveal?.name.trim();
    if (name != null &&
        name.isNotEmpty &&
    ...
    return 'CLIENT-${request.clientId.hashCode.abs() % 10000}';
  }

  void _syncSelectedClientFromText() {
    if (!mounted) return;
    final resolved = _resolveClientId(_clientIdController.text);
    if (resolved == _selectedClientId) return;
    setState(() => _selectedClientId = resolved);
  }

  String? _resolveClientId(String value) {
    final query = value.trim().toLowerCase();
    if (query.isEmpty) return null;
    for (final option in _connectedClientOptions) {
      if (option.clientId.toLowerCase() == query ||
          option.clientName.toLowerCase() == query ||
          option.displayText.toLowerCase() == query) {
  ...
  _ConnectedClientOption? _selectedClientOption() {
    final selectedId = _selectedClientId;
    if (selectedId == null) return null;
    for (final option in _connectedClientOptions) {
      if (option.clientId == selectedId) return option;
    }
    return null;
  }

  InputDecoration _clientSelectorDecoration({
    required bool isLoading,
    required bool hasError,
  }) {
    return InputDecoration(
      labelText: 'Share with connected client (Optional)',
      hintText: 'Search by client name, case, or ID',
  ...
  Widget _buildClientSelectorFallback() {
    return TextField(
      key: const Key('connected_client_selector_field'),
      controller: _clientIdController,
      decoration: _clientSelectorDecoration(isLoading: false, hasError: false),
    );
  }

  Widget _buildConnectedClientSelector({
    required bool isLoading,
    required bool hasError,
  }) {
    final selectedOption = _selectedClientOption();
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        RawAutocomplete<_ConnectedClientOption>(
          textEditingController: _clientIdController,
          focusNode: _clientSelectorFocusNode,
          displayStringForOption: (option) => option.displayText,
          optionsViewOpenDirection: OptionsViewOpenDirection.up,
          optionsBuilder: (value) {
            if (_connectedClientOptions.isEmpty) {
              return const <_ConnectedClientOption>[];
            }
            final query = value.text.trim().toLowerCase();
            if (query.isEmpty) return _connectedClientOptions;
            return _connectedClientOptions.where(
              (option) => option.matches(query),
            );
          },
          onSelected: (option) {
            setState(() {
              _selectedClientId = option.clientId;
              _clientIdController.value = TextEditingValue(
                text: option.displayText,
                selection: TextSelection.collapsed(
                  offset: option.displayText.length,
                ),
              );
            });
          },
          fieldViewBuilder:
              (context, textEditingController, focusNode, onFieldSubmitted) {
                return TextField(
                  ...
                  decoration: _clientSelectorDecoration(
                    isLoading: isLoading,
                    hasError: hasError,
                  ),
                );
                  ...
                  itemBuilder: (context, index) {
                    ...
                    return InkWell(
                      ...
                      child: Container(
                        ...
                        child: _connectedClientOptionTile(option),
                      ),
                    );
                  },
                ),
        ...
        if (hasError)
        ...
        else if (_connectedClientOptions.isEmpty && !isLoading)
          _clientSelectorHint(
          ...
          _clientSelectorHint(
            'Selected: ${selectedOption.clientName} • Client ID: ${selectedOption.clientId}',
            const Color(0xFF047857),
          )
        else
          _clientSelectorHint(
            '${_connectedClientOptions.length} connected client${_connectedClientOptions.length == 1 ? '' : 's'} available',
            const Color(0xFF64748B),
          ),
      ],
    );
  ...
  Widget _connectedClientOptionTile(_ConnectedClientOption option) {
    return Row(
      children: [
        Container(
          width: 36,
  ...
  Widget _clientSelectorHint(String text, Color color) {
    return Text(
      text,
      maxLines: 2,
      overflow: TextOverflow.ellipsis,
  ...
  Future<void> _onCompileAndVerify() async {
    ...
    final typedClient = _clientIdController.text.trim();
    final sharedClientId = _selectedClientId ?? _resolveClientId(typedClient);
    if (typedClient.isNotEmpty && sharedClientId == null) {
      _showErrorSnack('Select a connected client from the list first.');
      return;
    }
    ...
    try {
      ...
      final allowedUsers = sharedClientId != null && sharedClientId.isNotEmpty
          ? [sharedClientId]
          : <String>[];

      final String formattedFileName =
          '${_template['template_name']?.replaceAll(' ', '_') ?? 'contract'}_${DateTime.now().millisecondsSinceEpoch}.pdf';
      ...
      if (isTenancy) {
        final rentVal =
            double.tryParse(_controllers['f_monthly_rental']?.text ?? '') ??
            0.0;
        final secDeposit =
            double.tryParse(_controllers['f_security_deposit']?.text ?? '') ??
            0.0;
        final utilDeposit =
            double.tryParse(_controllers['f_utility_deposit']?.text ?? '') ??
            0.0;
        terms['monthlyRent'] = rentVal;
        terms['deposit'] = secDeposit + utilDeposit;
        terms['durationMonths'] =
            int.tryParse(_controllers['f_tenancy_duration']?.text ?? '') ?? 12;
      }

      terms['aiReview'] = {
        'riskLevel': 'Low',
        'riskColor': const Color(0xFF22C55E).toARGB32(),
        'riskCount': 0,
  ...
  Widget build(BuildContext context) {
    final fields = _template['dynamic_fields'] as List<dynamic>;
    final lawyerId = _currentLawyerId();
          ...
          child: Column(
            ...
            children: [
              ...
              Container(
                ...
                child: lawyerId == null
                    ? _buildClientSelectorFallback()
                    : StreamBuilder<List<ConnectionRequestModel>>(
                        stream: _connectionRequestRepository.streamForLawyer(
                          lawyerId,
                        ),
                        builder: (context, snapshot) {
                          final requests =
                              snapshot.data ?? const <ConnectionRequestModel>[];
                          _connectedClientOptions = _clientOptionsFromRequests(
                            requests,
                          );
                          return _buildConnectedClientSelector(
                            isLoading:
              ...
              const SizedBox(
                height: 100,
              ), // Pushing contents up for floating bottom button
            ],
          ),
        ),
      ),
...
class _ConnectedClientOption {
  final String clientId;
  final String clientName;
  final String caseTitle;

  const _ConnectedClientOption({
    required this.clientId,
    required this.clientName,
    required this.caseTitle,
  });

  String get displayText => '$clientName - $caseTitle';

  String get initials {
    final compact = clientName.replaceAll(RegExp(r'[^A-Za-z0-9]'), '');
    if (compact.isEmpty) return 'CL';
  ...
  bool matches(String query) {
    return clientName.toLowerCase().contains(query) ||
        clientId.toLowerCase().contains(query) ||
        caseTitle.toLowerCase().contains(query) ||
