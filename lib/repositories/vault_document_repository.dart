...
class VaultDocumentRepository {
  ...
  Future<Uint8List> _generateSignatureImage(List<Map<String, dynamic>> signaturePoints) async {
  ...
  Future<void> signContract({
    ...
    if (fileBytes != null && fileBytes.isNotEmpty && (model.contentType == 'application/pdf' || model.fileName.toLowerCase().endsWith('.pdf'))) {
      try {
        // 2. Load PDF
        final PdfDocument document = PdfDocument(inputBytes: fileBytes);
        
        // 3. Generate Signature Image
        final Uint8List sigImageBytes = await _generateSignatureImage(signaturePoints);
        final PdfBitmap signatureBitmap = PdfBitmap(sigImageBytes);
        
        // 4. Determine page and location
        final aiReview = docData['contractTerms']?['aiReview'] as Map<String, dynamic>?;
        ...
        if (aiReview != null) {
           ...
           if (sigPage is int) pageIndex = sigPage - 1; // 1-indexed to 0-indexed
           if (aiReview['signatureAnchor'] is String) anchorText = aiReview['signatureAnchor'];
           if (aiReview['signatureOffset'] is String) offset = aiReview['signatureOffset'];
        }
        ...
        final List<TextLine> lines = extractor.extractTextLines(startPageIndex: pageIndex);
        ...
        for (final line in lines) {
           ...
           if (isMatch) {
               ...
               if (offset == 'right') {
                 ...
                 stampRect = ui.Rect.fromLTWH(
                   bounds.left, 
                   bounds.top - imgHeight - 2, // reduced gap to stay closer to the designated line
                   imgWidth, 
                   imgHeight
                 );
               }
        ...
        page.graphics.drawString(
           ...
           bounds: ui.Rect.fromLTWH(stampRect.left, stampRect.bottom + 2, stampRect.width, 30),
        ...
        await _storage.ref(newStoragePath).putData(
          newBytes,
          SettableMetadata(contentType: 'application/pdf')
        );
        final String newDownloadUrl = await _storage.ref(newStoragePath).getDownloadURL();
